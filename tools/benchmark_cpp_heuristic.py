...
def run_cpp(
    ...
    *,
    ...
    if proc.returncode not in (0, 1):
        raise RuntimeError(
            f"C++ run failed for {case.name} ({proc.returncode}): {proc.stderr.strip()}"
        )
...
def witness_delta(payload: Mapping[str, object]) -> int:
    if not payload.get("simplification_found"):
        return 0
    red_path = payload.get("red_path", [])
    green_path = payload.get("green_path", [])
    if not isinstance(red_path, list) or not isinstance(green_path, list):
        return 0
    return max(0, len(red_path) - len(green_path))


def output_crossings(payload: Mapping[str, object], fallback: int) -> int:
    pd_simplification = payload.get("pd_simplification")
    if isinstance(pd_simplification, dict):
        value = pd_simplification.get("output_crossings")
...
def reduction_crossings(case: BenchmarkCase, payload: Mapping[str, object]) -> int:
    after_preprocessing = output_crossings(payload, case.crossings)
    return max(0, case.crossings - after_preprocessing + witness_delta(payload))
        ...
        for case in cases:
            for mode, ban_heuristic in modes:
                ...
                row: RawRow = {
                    ...
                    "witness_delta": witness_delta(payload),
                    ...
                    "path_search_mode": str(payload.get("path_search_mode", "")),
                }
                rows.append(row)
                print(
    ...
    *,
    ...
    fig.text(
        ...
        (
            ...
            f"{repeat} repeat(s). Brute force took {time_ratio:.1f}x heuristic time."
        ),
        ha="center",
        va="bottom",
        fontsize=8.5,
...
def main(argv: Optional[Sequence[str]] = None) -> int:
    ...
    rows = benchmark(cpp_exe, cases, args.repeat, timeout)
    ...
    if args.plot:
        plot_summary(args.plot, summary, repeat=args.repeat, case_count=len(cases))
    return 0
